...
<%
    ...
    if (errmsg.isEmpty()) {
        ...
        if (orgorder == null) {
            ...
            } else if (actiontype != null && actiontype.equals("save")) {
                order.setAddress(address);
                order.setCreditCard(creditcard);
                if (address == null || address.isEmpty()) {
                    errmsg = "Address can't be empty!";
                } else if (creditcard == null || creditcard.isEmpty()) {
                    errmsg = "Credit card can't be empty!";
                } else if (order != null) {
...
<%
    ...
    String user_card = request.getParameter("creditcard");
    //String name="Advait007";
    try {
        // create a java mysql database connection
        String myDriver = "com.mysql.jdbc.Driver";
        String myUrl = "jdbc:mysql://localhost:3308/test";
        Class.forName(myDriver);
        Connection conn = DriverManager.getConnection(myUrl, "root", "haresh");

        // create the java mysql update preparedstatement
        String query = "update customer_info set EMAIL_ID = ?, ADDRESS = ?, CONTACT_NO = ?, CARD_NO = ? where FULL_NAME = ?";

        PreparedStatement preparedStmt = conn.prepareStatement(query);
        preparedStmt.setString(1, user_email);
        preparedStmt.setString(2, user_address);
        preparedStmt.setString(3, user_contact);
        preparedStmt.setString(4, user_card);
        preparedStmt.setString(5, "Mahendra Singh Dhoni");
        ...
        <div class="order_box">
            <form action="admin_orderedit.jsp?actiontype=save" method="Post">
                ...
                <table class="order_table">               
                    ...
                    <tr><td><h5><i>Email ID: </i></h5></td><td><input type='text' name='email_id' id='address' value='${order.address}' required /></td><td></td></tr>
                    <tr><td><h5><i>Address: </i></h5></td><td><input type='text' size="40" name='address' id='address' value='${order.address}' required /></td><td></td></tr>
                    <tr><td><h5><i>Contact Number: </i></h5></td><td><input type='text' name='contact' id='address' value='${order.address}' required /></td><td></td></tr>
